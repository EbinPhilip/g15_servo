    ...
    -t1).count() < wait_ms)
    {
        for (int i = 1; i<=3;++i)
        {
            ...
            if ( !servo.readInstruction(i, PRESENT_POSITION_L, read_bytes, 4) )
            {
                ...
                std::cout<<("Pos: ");
                std::cout<<(servo.convertFromRegisterValue(value, POSITION_MAX_REGISTER, 360.0), 2);
                std::cout<<(" degrees");
                std::cout<<(" | ");
                value = read_bytes[3]&0x01;
                value = (value << 8) | read_bytes[2];
                std::cout<<("Speed: ");
                std::cout<<(servo.convertFromRegisterValue(value, SPEED_MAX_REGISTER, SPEED_MAX_RPM), 2);
                std::cout<<(" rpm");
            }
            else
            {
